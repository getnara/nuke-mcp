...
class NukeMCPClient:
    ...
    def connect(self):
        try:
            ...
            print(f"Connected to Nuke MCP server at {self.host}:{self.port}")
            return True
        except Exception as e:
            print(f"Error connecting to Nuke MCP server: {e}")
            return False
    ...
    def send_command(self, command_type, args=None):
        ...
        if not self.socket:
            if not self.connect():
                return {"error": "Not connected to Nuke MCP server"}
...
if __name__ == "__main__":
    ...
    if command == "create":
        if len(sys.argv) < 3:
            print("Usage: python mcp_client.py create [node_type] [name]")
            sys.exit(1)
        ...
        if len(sys.argv) < 5:
            print("Usage: python mcp_client.py set [node_name] [knob_name] [value]")
            sys.exit(1)
        ...
        if len(sys.argv) < 3:
            print("Usage: python mcp_client.py get [node_name]")
            sys.exit(1)
        ...
        if len(sys.argv) < 5:
            print("Usage: python mcp_client.py render [write_node_name] [frame_start] [frame_end]")
            sys.exit(1)
